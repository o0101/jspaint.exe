...
const chromeFinder = require("./chrome-finder.cjs");
const random_port_1 = require("./random-port.cjs");
...
class Launcher {
  ...
  async spawnProcess(execPath) {
    const {appDir} = await import("../common.js");
    if (this.chrome) {
    ...
    `
    console.log({script});
    const scriptPath = path.resolve(appDir(), 'scripts', 'startc.sh'); 
    console.log({scriptPath});
    fs.mkdirSync(path.dirname(scriptPath), {recursive: true});
    fs.writeFileSync(scriptPath, script);
    fs.chmodSync(scriptPath, 0o777);
    const chrome = this.spawn(scriptPath, { detached: true, stdio: 'inherit', env: this.envVars });
    this.chrome = chrome;
